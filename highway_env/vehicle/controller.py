...
class MDPNoColVehicle(ControlledVehicle):
    ...
    SPEED_MIN: float = 20  # [m/s]
    SPEED_MAX: float = 30  # [m/s]
    CRASHED_FLAG: bool = False
    CURRENT_CRASH: bool = False
    CURRENT_DIST: float = 10 # m
    ...
    def handle_collisions(self, other: 'RoadObject', dt: float = 0) -> None:
        ...
        if intersecting or will_intersect:
            ...
            self.CURRENT_CRASH = True
            self.CURRENT_DIST = np.linalg.norm(self.position - other.position)
        # if will_intersect:
        #     if self.solid and other.solid:
        #         self.impact = None #transition / 2
        #         other.impact = None #-transition / 2
        # if intersecting:
        #     if self.solid and other.solid:
        #         self.crashed = True
        #         other.crashed = True
