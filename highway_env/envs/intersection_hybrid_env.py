...
class IntersectionHybridEnv(AbstractEnv):
    ...
    def _reset(self) -> None:
        ...
        self._make_pedestrians(self.config["initial_pedestrian_count"])

    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        obs, reward, done, info = super().step(action)
        self._clear_vehicles()
        self._clear_pedestrians()
        self._spawn_vehicle(spawn_probability=self.config["spawn_probability"])
        self._spawn_pedestrians(spawn_probability=self.config["spawn_probability"])
    ...
    def _make_road(self) -> None:
        ...
        for corner in range(4):
            ...
            net.add_lane("o" + str(corner), "ir" + str(corner),
                         StraightLane(start, end, lane_width, line_types=[s, c], priority=priority, speed_limit=10))
            # Right turn
            ...
            net.add_lane("ir" + str(corner), "il" + str((corner - 1) % 4),
                         CircularLane(r_center, right_turn_radius, angle + np.radians(180), angle + np.radians(270),
                                      width=lane_width, line_types=[n, c], priority=priority, speed_limit=10))
            # Left turn
            ...
            net.add_lane("ir" + str(corner), "il" + str((corner + 1) % 4),
                         CircularLane(l_center, left_turn_radius, angle + np.radians(0), angle + np.radians(-90),
                                      width=lane_width, clockwise=False, line_types=[n, n], priority=priority - 1, speed_limit=10))
            # Straight
            start = rotation @ np.array([lane_width / 2, outer_distance])
            end = rotation @ np.array([lane_width / 2, -outer_distance])
            net.add_lane("ir" + str(corner), "il" + str((corner + 2) % 4),
                         StraightLane(start, end, lane_width, line_types=[s, n], priority=priority, speed_limit=10))
            # Exit
            ...
            net.add_lane("il" + str((corner - 1) % 4), "o" + str((corner - 1) % 4),
                         StraightLane(end, start, lane_width, line_types=[n, c], priority=priority, speed_limit=10))
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            # ego_vehicle = self.action_type.vehicle_class(
            #     self.road,
            #     ego_lane.position(80 + 5 * self.np_random.randn(1), 0),
            #     speed=ego_lane.speed_limit,
            #     heading=ego_lane.heading_at(60))
            try:
    ...
    def _make_pedestrians(self, n_pedestrians: int = 5) -> None:
        """
        Populate a road with several pedestrians on the intersection and on the merging lane
        ...
        # Random pedestrians
        simulation_steps = 3
        for t in range(n_pedestrians - 1):
    ...
    def _spawn_pedestrians(self,
                       ...
                       spawn_probability: float = 0.6,
                       go_straight: bool = False) -> None:
        if self.np_random.rand() > spawn_probability  or len(self.road.pedestrians) >= 5:
            return

        route = self.np_random.choice(range(4), size=2, replace=False)
        route[1] = (route[0] + 2) % 4 if go_straight else route[1]

        position = [x * np.sign(np.random.rand()-0.5), y * np.sign(np.random.rand()-0.5)]
        idx = np.random.randint(0, 2)
        target = deepcopy(position)
        target[idx] *= -1
        pedestrian = GeneralPedestrian(self.road, position, target)
        # for v in self.road.vehicles:
        #     if np.linalg.norm(v.position - vehicle.position) < 15:
        #         return
        # vehicle.plan_route_to("o" + str(route[1]))
        pedestrian.randomize_behavior()
    ...
    def _clear_pedestrians(self) -> None:
        def is_leaving(ped):
            vec = np.array([ped.position[0] - ped.target[0], ped.position[1] - ped.target[1]])
            heading_vec = np.array([np.cos(ped.heading), np.sin(ped.heading)])
            is_leaving = True if np.dot(vec, heading_vec) > 0 else False
            return is_leaving
        self.road.pedestrians = [ped for ped in self.road.pedestrians if
                                 not is_leaving(ped)]
...
register(
    id='intersection-hybrid-v0',
    entry_point='highway_env.envs:IntersectionHybridEnv',
)